	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			fixHeight: function (newH) {
				...
				if (nd > newH) $(g.nDiv).height(160).width(200);
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if (p.onDoubleClick) {
						alert("aqui va el codigo del doble click en flexigrid.js linea 808");
						p.onDoubleClick(this, g, p);
					}
				}).bind( "taphold", function(){
					alert("aqui va el codigo del click presionado en el flexigrid.js linea 813");
					...
					$("#menuBotones").css({'display':'block', 'left':e.pageX, 'top':e.pageY});
					return false;
				}).hover(function (e) {
					if (e.shiftKey) {
						$(this).toggleClass('trSelected');
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			$(this).empty().append(thdiv).removeAttr('width').mousedown(function (e) {
				...
				if (g.colCopy) {
					...
					var bDivAlt = jQuery(g.bDiv).outerHeight();
					var pDivAlt = jQuery(g.pDiv).outerHeight();
					var alt = jQuery(g.nBtn).outerHeight();
					$(g.nDiv).hide();
					$(g.nBtn).hide();
					/*alert($('div:eq(' + nv + ')', g.cDrag).css('left'));
					alert(this.children[0].style.width);*/ 
					$(g.nBtn).css({
						'left': nl + (nv * 2) + 2,
						top: ((bDivAlt + pDivAlt) * -1) - alt
					}).show();
